...
| Decision | Choice |
|---|---|
| Price | **₹99**, a single introductory offer. No plan/SKU concept — the landing page's Save the Date and bundle plans stay WhatsApp-booked. |
...
- The charged amount is `PRICE_PAISE` in `modules/self-serve/pricing.ts`, read
  **server-side only**. It is never a parameter to `createOrder` and never
  ...
  crafted request away from selling a ₹99 product for ₹1.
...
`PRICE` (`components/landing/data.ts`) is ₹99 and `PRICE_WAS` is ₹1,599, shown
...
don't describe a choice next to a ₹99 full invitation. To end the offer, restore
