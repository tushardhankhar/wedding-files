...
`purchase` carries `value` (499), `currency` (INR), `transaction_id` and
`event_id` (both the Razorpay payment id), plus an `items` array.

**Value is in rupees, not paise.** Sending 49900 would tell Meta each sale is
worth ₹49,900 and let it bid roughly a hundred times what a buyer is actually
...
₹499 product the temptation is to optimise for something that fires more often,
