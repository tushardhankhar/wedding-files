        ...
        <section id="rsvp" className="scroll-mt-20 px-6 py-28 sm:px-10" style={{ background: "linear-gradient(180deg,#fff8e8,#f5e6c8)" }}>
          <div className="mx-auto max-w-2xl text-center">
            ...
            <div className="mt-12 text-left">
              {rsvp ? <AkaGroupRsvp slug={rsvp.slug} events={events} existing={rsvp.existing} /> : selfRsvp ? <AkaSelfRsvp slug={selfRsvp.slug} events={selfRsvp.events} existing={selfRsvp.existing} /> : <AkaRsvpDemo events={events} />}
            </div>
          </div>
        </section>
      ) : null}
...
function AkaGroupRsvp({ slug, events, existing }: { slug: string; events: WebsiteViewProps["events"]; existing: ExistingGroupRsvp }) {
  const r = useGroupRsvp(slug, events, existing);
  const numField = "w-20 border border-[color:var(--a-gold)]/50 bg-transparent px-3 py-2 text-center text-[color:var(--a-charcoal)] focus:border-[color:var(--a-saffron)] focus:outline-none";

  if (r.done) {
    const summary = events.filter((e) => r.entries[e.id]?.attending).map((e) => `${e.name}: ${r.entries[e.id].partySize}`).join(" · ");
    return (
      <div className="space-y-3 text-center">
        <p className="a-serif text-2xl italic text-[color:var(--a-burgundy)]"><TT en="Thank you. Your love means more than we can say." hi="धन्यवाद। आपका प्रेम शब्दों से परे है।" /></p>
        <p className="text-sm text-[color:var(--a-ink-soft)]">{summary || <TT en="Not attending" hi="नहीं आ रहे" />}</p>
        <button type="button" onClick={r.edit} className="text-[11px] uppercase tracking-[0.25em] text-[color:var(--a-ink-soft)] underline underline-offset-4 hover:text-[color:var(--a-saffron)]"><TT en="Edit my RSVP" hi="उत्तर बदलें" /></button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-10">
        ...
        return (
          <div key={e.id}>
            ...
            <div className="mx-auto mt-4 flex max-w-md flex-wrap items-center justify-center gap-2">
              <AkaChoice on={en.attending} tone="yes" onClick={() => r.setAttending(e.id, true)}><TT en="Yes, with joy" hi="जी, सहर्ष" /></AkaChoice>
              <AkaChoice on={!en.attending} tone="no" onClick={() => r.setAttending(e.id, false)}><TT en="Unable to attend" hi="नहीं आ पाएँगे" /></AkaChoice>
              {en.attending ? (
                <label className="flex items-center gap-2 text-sm text-[color:var(--a-ink-soft)]">
      ...
      <button type="button" onClick={r.submit} disabled={r.pending} className="w-full bg-[color:var(--a-saffron)] px-6 py-4 text-[11px] font-semibold uppercase tracking-[0.25em] text-white disabled:opacity-60">{r.pending ? "…" : r.saved ? <TT en="Save changes" hi="बदलाव सहेजें" /> : <TT en="Send our response" hi="उत्तर भेजें" />}</button>
    </div>
  );
}

function AkaSelfRsvp({ slug, events, existing }: { slug: string; events: { id: string; name: string }[]; existing?: ExistingSelfRsvp | null }) {
  const r = useSelfRsvp(slug, events.map((e) => e.id), existing);
  if (r.done) return (
    <div className="space-y-3 text-center">
      <p className="a-serif text-center text-2xl italic text-[color:var(--a-burgundy)]"><TT en="Thank you. Your love means more than we can say." hi="धन्यवाद। आपका प्रेम शब्दों से परे है।" /></p>
      <p className="text-sm text-[color:var(--a-ink-soft)]">{r.savedRsvp?.name} · {r.savedRsvp?.partySize} <TT en="guest(s)" hi="अतिथि" /></p>
      <button type="button" onClick={r.edit} className="text-[11px] uppercase tracking-[0.25em] text-[color:var(--a-ink-soft)] underline underline-offset-4 hover:text-[color:var(--a-saffron)]"><TT en="Edit my RSVP" hi="उत्तर बदलें" /></button>
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-md space-y-4">
      ...
      <button type="button" onClick={r.submit} disabled={r.pending} className="w-full bg-[color:var(--a-saffron)] px-6 py-4 text-[11px] font-semibold uppercase tracking-[0.25em] text-white disabled:opacity-60">{r.pending ? "…" : r.savedRsvp ? <TT en="Save changes" hi="बदलाव सहेजें" /> : <TT en="Send our response" hi="उत्तर भेजें" />}</button>
    </div>
  );
}
    ...
    <div className="space-y-8">
      {events.slice(0, 2).map((e) => (
        <div key={e.id}>
          ...
          <div className="mx-auto mt-4 flex max-w-md flex-wrap items-center justify-center gap-2">
            <AkaChoice on tone="yes" disabled><TT en="Yes, with joy" hi="जी, सहर्ष" /></AkaChoice>
            <AkaChoice on={false} tone="no" disabled><TT en="Unable to attend" hi="नहीं" /></AkaChoice>
            <span className="border border-[color:var(--a-gold)]/50 px-4 py-2 text-sm text-[color:var(--a-ink-soft)]"><TT en="2 guests" hi="2 अतिथि" /></span>
          </div>
        </div>
      ))}
      <p className="text-center text-sm italic text-[color:var(--a-ink-soft)]"><TT en="Your families will RSVP with a headcount here." hi="आपके परिवार यहाँ संख्या के साथ उत्तर देंगे।" /></p>
    </div>
  );
}
